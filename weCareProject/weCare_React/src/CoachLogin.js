...
class CoachLogin extends React.Component {
    ...
    handleLogin = (e) => {
        e.preventDefault();
        axios.post("http://localhost:4000/coaches/login",{
            id:this.state.id,
        ...
        .then((result) => {
            ...
            if (result.status == 200) {
                ...
                this.props.history.replace({pathname:"/coachhome",state:{id:this.state.id}});
                //this.props.history.replace("/coachJoin");
            }
            else {
                this.setState({errorFromServer:result})
                console.log(result)
            };
        })
        .catch(err => {
    ...
    render () {
        return (
            <React.Fragment>
                <Navbar page={"coachLogin"} />
                <form className = "card-coach-login">
                    <div>
                        <label for = "id" className = "form-coach-login">userId : </label>
                        <input name = "id" type="text" onChange={this.updateTextField} value={this.id}/>
                        {(this.state.initialEditFlags[0] == false || this.state.idValidator)  ? <p></p>:<p className = "myerror1-coach-login">This is a required field</p>}
                    </div>
                    <div>
                        <label for = "password" className = "form-coach-login">Password : </label>
                        <input name= "password" type="password" onChange={this.updateTextField} value={this.password}/><br />
                        {(this.state.initialEditFlags[1] == false || this.state.passwordValidator) ? <p></p>:<p className = "myerror1-coach-login">Password must be 5 to 10 characters long</p>}
                    </div>
                    <input type="submit" value="submit" disabled={!(this.state.idValidator && this.state.passwordValidator)} onClick={this.handleLogin}/>
                    {this.state.errorFromServer ? <div><p className = "myerror1-coach-login">Status : {this.state.errorFromServer.status}</p><p className = "myerror1-coach-login">Message : {this.state.errorFromServer.data.message}</p></div> : <div><p></p></div>}
                </form>
            </React.Fragment>
        );
    }
